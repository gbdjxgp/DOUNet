...
class FCBL(nn.Module):
    def __init__(self,
        ...
        # initial variables
        # logger = get_root_logger()
        # logger_instance.log(logging.INFO, )
    ...
    def forward(self,
        ...
        if self.use_sigmoid is True:
            ...
            cls_loss = F.cross_entropy(cls_score, label,
                                       reduction='none')
            cls_loss = torch.sum(weight * cls_loss) / self.n_i
    ...
    def get_cls_channels(self, cls_score):
        return self.get_channel_num(cls_score)




if __name__ == '__main__':
    cls_score = torch.randn(1024, 11)#10前景+1背景
    label = torch.randint(0,11,(1024,))
    fcbl = FCBL(num_classes=10)#0-9为前景(10)，10为背景
    loss = fcbl(cls_score, label)
    print("done")
